...
def train_model_task(self, dataset_id, params):
    try:
        ...
        numeric = df.select_dtypes(include=[np.number]).fillna(0).replace([np.inf, -np.inf], 0)
        scaler = StandardScaler()
        X = scaler.fit_transform(numeric)

        model = IsolationForest(n_estimators=params.get('n_estimators', 100), contamination=params.get('contamination', 0.01), random_state=42)
        model.fit(X)
        ...
        artifact.metrics = {'anomaly_count': int(df['anomaly'].sum())}
        artifact.save()
        ds.status = 'done'
        ds.meta = {'rows': len(df)}
        ds.save()
        return {'status': 'success', 'artifact_id': str(artifact.id)}
